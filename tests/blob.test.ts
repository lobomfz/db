...
describe("blob", () => {
	...
	test("round-trips Uint8Array through insert and select", async () => {
		...
		const row = await db.kysely.selectFrom("files").selectAll().executeTakeFirstOrThrow();
	...
	test("round-trips Int8Array stored as Uint8Array view", async () => {
		...
		const row = await db.kysely.selectFrom("envelopes").selectAll().executeTakeFirstOrThrow();

		const recovered = new Int8Array(row.data.buffer, row.data.byteOffset, row.data.byteLength);
